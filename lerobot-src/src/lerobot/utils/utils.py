...
def get_safe_torch_device(try_device: str, log: bool = False) -> torch.device:
    ...
    if try_device.startswith("cuda"):
        ...
        assert _torch_xpu_is_available()
...
def get_safe_dtype(dtype: torch.dtype, device: str | torch.device):
    ...
    if device == "xpu" and dtype == torch.float64:
        xpu = _torch_xpu_backend()
        if xpu is not None and hasattr(xpu, "get_device_capability"):
            device_capability = xpu.get_device_capability()
...
def is_torch_device_available(try_device: str) -> bool:
    ...
    if try_device.startswith("cuda"):
        ...
        return _torch_xpu_is_available()
    elif try_device == "cpu":
        return True
    else:
