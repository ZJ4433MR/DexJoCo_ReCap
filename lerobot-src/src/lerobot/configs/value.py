...
class ValueInferenceACPConfig:
    ...
    binarization: str = "task_quantile"
    multitag_ratios: str = "0.1,0.2,0.3"
    min_positive_run_length: int = 1
    success_aware: bool = False
    ...
    def validate(self) -> None:
        ...
        valid_binarizations = {"task_quantile", "episode_topk_smooth", "episode_multitag_smooth"}
        if self.binarization not in valid_binarizations:
            raise ValueError(f"'acp.binarization' must be one of {sorted(valid_binarizations)}.")
        if self.binarization == "episode_multitag_smooth":
            ratios = [v.strip() for v in self.multitag_ratios.split(",") if v.strip()]
            if len(ratios) != 3:
                raise ValueError(
                    "'acp.multitag_ratios' must contain three comma-separated fractions for high,medium,low."
                )
            parsed = [float(v) for v in ratios]
            if any(v < 0.0 for v in parsed) or sum(parsed) > 1.0:
                raise ValueError("'acp.multitag_ratios' entries must be non-negative and sum to <= 1.")
        if self.min_positive_run_length <= 0:
            raise ValueError("'acp.min_positive_run_length' must be > 0.")
