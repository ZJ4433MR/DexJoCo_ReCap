...
def build_acp_tagged_task(
    task: str | None,
    is_positive: bool | None = None,
    *,
    tag_value: str | None = None,
    tag_key: str = ACP_TAG_KEY,
) -> str:
    if tag_value is None:
        if is_positive is None:
            raise ValueError("Either 'is_positive' or 'tag_value' must be provided.")
        tag_value = ACP_POSITIVE_VALUE if is_positive else ACP_NEGATIVE_VALUE

    tag_value = str(tag_value).strip()
    if not tag_value:
        raise ValueError("ACP tag value must be non-empty.")
    tag_key = str(tag_key).strip() or ACP_TAG_KEY
