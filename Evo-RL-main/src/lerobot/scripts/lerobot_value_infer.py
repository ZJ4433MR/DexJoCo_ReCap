...
def _remove_short_positive_runs(
    ...
    for ep_idx in np.unique(episode_indices):
        positions = np.flatnonzero(episode_indices == ep_idx)
        if positions.size == 0:
            continue

        start: int | None = None
        for local_i, absolute_i in enumerate(positions):
            if indicators[absolute_i] == 1 and start is None:
                start = local_i
            is_run_end = start is not None and (
                indicators[absolute_i] == 0 or local_i == positions.size - 1
            )
            if not is_run_end:
                continue

            end = local_i if indicators[absolute_i] == 0 else local_i + 1
            if end - start < min_run_length:
                smoothed[positions[start:end]] = 0
            start = None if indicators[absolute_i] == 0 else start

    return smoothed


def _remove_short_nonzero_runs(
    tags: np.ndarray,
    episode_indices: np.ndarray,
    min_run_length: int,
) -> np.ndarray:
    ...
    for ep_idx in np.unique(episode_indices):
        ...
        start: int | None = None
        for local_i, absolute_i in enumerate(positions):
            if tags[absolute_i] > 0 and start is None:
                start = local_i
            is_run_end = start is not None and (tags[absolute_i] == 0 or local_i == positions.size - 1)
            if not is_run_end:
                continue

            end = local_i if tags[absolute_i] == 0 else local_i + 1
            if end - start < min_run_length:
                smoothed[positions[start:end]] = 0
            start = None if tags[absolute_i] == 0 else start

    return smoothed


def _binarize_advantages_episode_topk_smooth(
    episode_indices: np.ndarray,
    advantages: np.ndarray,
    episode_info: dict[int, EpisodeTargetInfo],
    positive_ratio: float,
    ...
    if positive_ratio > 0.0:
        for ep_idx in np.unique(episode_indices):
            ...
            if success_aware and not episode_info[int(ep_idx)].success:
                # In failed trajectories, keep only locally improving frames eligible
                # for ACP-positive labels instead of forcing a fixed positive count.
                candidate_positions = positions[advantages[positions] > 0.0]
            if candidate_positions.size == 0:
                continue

            k = max(1, int(round(float(candidate_positions.size) * positive_ratio)))
            k = min(k, candidate_positions.size)
            order = np.argsort(-advantages[candidate_positions], kind="stable")
            indicators[candidate_positions[order[:k]]] = 1

    indicators = _remove_short_positive_runs(
        indicators=indicators,
        episode_indices=episode_indices,
        min_run_length=min_positive_run_length,
    )
...
def _parse_multitag_ratios(raw: str) -> tuple[float, float, float]:
    values = [float(value.strip()) for value in raw.split(",") if value.strip()]
    if len(values) != 3:
        raise ValueError("'multitag_ratios' must have three comma-separated values: high,medium,low.")
    if any(value < 0.0 for value in values) or sum(values) > 1.0:
        raise ValueError("'multitag_ratios' entries must be non-negative and sum to <= 1.")
...
def _ratio_count(total: int, ratio: float) -> int:
    if total <= 0 or ratio <= 0.0:
        return 0
    return max(1, int(round(float(total) * ratio)))


def _label_advantages_episode_multitag_smooth(
    episode_indices: np.ndarray,
    advantages: np.ndarray,
    episode_info: dict[int, EpisodeTargetInfo],
    multitag_ratios: str,
    ...
    high_ratio, medium_ratio, low_ratio = _parse_multitag_ratios(multitag_ratios)
    tags = np.zeros_like(advantages, dtype=np.int64)
    ...
    for ep_idx in np.unique(episode_indices):
        ...
        order = candidate_positions[np.argsort(-advantages[candidate_positions], kind="stable")]
        remaining = order.size
        high_count = min(_ratio_count(remaining, high_ratio), remaining)
        medium_count = min(_ratio_count(remaining, medium_ratio), remaining - high_count)
        low_count = min(_ratio_count(remaining, low_ratio), remaining - high_count - medium_count)

        start = 0
        if high_count:
            tags[order[start : start + high_count]] = 3
            start += high_count
        if medium_count:
            tags[order[start : start + medium_count]] = 2
            start += medium_count
        if low_count:
            tags[order[start : start + low_count]] = 1

    tags = _remove_short_nonzero_runs(
        tags=tags,
        episode_indices=episode_indices,
        min_run_length=min_positive_run_length,
    )
...
def run_value_inference_pipeline(
    ...
    if accelerator.is_main_process:
        ...
        if cfg.acp.enable:
            ...
            if cfg.acp.binarization == "task_quantile":
            ...
            elif cfg.acp.binarization == "episode_topk_smooth":
                thresholds = None
                indicators = _binarize_advantages_episode_topk_smooth(
                    episode_indices=episode_indices,
                    advantages=advantages,
                    episode_info=episode_info,
                    positive_ratio=cfg.acp.positive_ratio,
                    min_positive_run_length=cfg.acp.min_positive_run_length,
                    success_aware=cfg.acp.success_aware,
                    interventions=interventions,
                    force_intervention_positive=cfg.acp.force_intervention_positive,
                )
            elif cfg.acp.binarization == "episode_multitag_smooth":
                thresholds = None
                indicators = _label_advantages_episode_multitag_smooth(
                    episode_indices=episode_indices,
                    advantages=advantages,
                    episode_info=episode_info,
                    multitag_ratios=cfg.acp.multitag_ratios,
                    min_positive_run_length=cfg.acp.min_positive_run_length,
                    success_aware=cfg.acp.success_aware,
                    interventions=interventions,
                    force_intervention_positive=cfg.acp.force_intervention_positive,
                ...
                raise ValueError(f"Unsupported ACP binarization mode: {cfg.acp.binarization}")

            indicator_positive_ratio = float(np.mean((indicators > 0).astype(np.float32)))
            tag_counts = {
                int(tag_id): int(count)
                for tag_id, count in enumerate(np.bincount(indicators.astype(np.int64), minlength=4))
                if count
            }
            logging.info(
                "ACP stats | n_step=%d binarization=%s positive_ratio_target=%.4f "
                "positive_ratio_observed=%.4f min_positive_run_length=%d success_aware=%s tag_counts=%s",
                cfg.acp.n_step,
                cfg.acp.binarization,
                cfg.acp.positive_ratio,
                indicator_positive_ratio,
                cfg.acp.min_positive_run_length,
                cfg.acp.success_aware,
                tag_counts,
            )
        ...
        result = {
            ...
            "thresholds": thresholds,
            "acp_binarization": cfg.acp.binarization,
            "acp_multitag_ratios": cfg.acp.multitag_ratios,
            "acp_min_positive_run_length": cfg.acp.min_positive_run_length,
            "acp_success_aware": cfg.acp.success_aware,
            "viz_outputs": viz_outputs,
        }
    else:
        result = {
