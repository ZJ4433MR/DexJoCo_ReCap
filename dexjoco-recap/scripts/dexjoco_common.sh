...
dexjoco_nodefaults_env_file() {
  local env_file="$1"
  local out_file="$2"
  python - "$env_file" "$out_file" <<'PY'
...
in_channels = False
seen_channels = False
seen_nodefaults = False

def finish_channels():
    global seen_nodefaults
    if not seen_nodefaults:
        out.append("  - nodefaults")
        seen_nodefaults = True
...
for line in lines:
    ...
    if in_channels:
        if stripped.startswith("- "):
            channel = stripped[2:].strip().strip("'\"")
            if channel == "defaults":
                continue
...
if not seen_channels:
    out = ["channels:", "  - conda-forge", "  - nodefaults"] + out

dst.parent.mkdir(parents=True, exist_ok=True)
dst.write_text("\n".join(out) + "\n")
print(f"[dexjoco] conda env file patched for nodefaults: {dst}")
PY
}
...
dexjoco_conda_env_create_retry() {
  ...
  local rc=0
  RUN_ROOT="${RUN_ROOT:-$(_dexjoco_run_root)}"
  local create_env_file="$env_file"

  if [[ "${DEXJOCO_CONDA_NODEFAULTS:-1}" == "1" ]]; then
    create_env_file="$(dirname "$env_file")/.recap-nodefaults-$(basename "$env_file")"
    dexjoco_nodefaults_env_file "$env_file" "$create_env_file"
  fi

  for attempt in 1 2 3; do
    echo "[dexjoco] conda env create attempt $attempt: $env_prefix"
    if conda env create --prefix "$env_prefix" -f "$create_env_file"; then
      return 0
    fi
    rc=$?
