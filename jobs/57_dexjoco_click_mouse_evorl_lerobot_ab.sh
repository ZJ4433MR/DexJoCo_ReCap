...
DEXJOCO_EVO_SOURCE_WORKSPACE_TAR="${DEXJOCO_EVO_SOURCE_WORKSPACE_TAR:-/share/home/u23133/.cache/recap-sim-l40-stage/dexjoco_click_mouse_p2_frozen_pool_ablation_l40_v4/incoming.tar.gz}"
...
if [[ "$DEXJOCO_EVO_EVAL_PROMPT" != "positive" && "$DEXJOCO_EVO_EVAL_PROMPT" != "base" && "$DEXJOCO_EVO_EVAL_PROMPT" != "negative" ]]; then
  echo "[evorl-lerobot] DEXJOCO_EVO_EVAL_PROMPT must be 'positive', 'negative', or 'base'" >&2
  exit 2
fi
...
if [[ "$DEXJOCO_EVO_ACP_BINARIZATION" != "task_quantile" && "$DEXJOCO_EVO_ACP_BINARIZATION" != "episode_topk_smooth" ]]; then
  echo "[evorl-lerobot] DEXJOCO_EVO_ACP_BINARIZATION must be 'task_quantile' or 'episode_topk_smooth'" >&2
  exit 2
fi
...
if [[ ! -d "$EXP_DIR/.local/dexjoco-src/openpi" && -f "$DEXJOCO_EVO_SOURCE_WORKSPACE_TAR" ]]; then
  echo "[evorl-lerobot] restoring packaged DexJoCo fallback from $DEXJOCO_EVO_SOURCE_WORKSPACE_TAR"
  mkdir -p "$EXP_DIR/.local"
  tar -xzf "$DEXJOCO_EVO_SOURCE_WORKSPACE_TAR" -C "$EXP_DIR/.." ./recap-sim-l40/.local/dexjoco-src
fi
...
conda run --no-capture-output --prefix "$DEXJOCO_ENV_PREFIX" python - "$base_config" "$positive_eval_config" "$negative_eval_config" "$DEXJOCO_ACP_SUFFIX" <<'PY'
